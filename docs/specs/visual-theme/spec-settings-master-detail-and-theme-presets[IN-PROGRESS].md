...
- [ ] **需用户手动验证**（本次执行者无屏幕录制权限，无法自行截图确认）：
  点击右上角齿轮进入设置页后，看到左侧设置项列表（6 项：外观主题、
  强调色、窗口材质、默认到期偏移、标签管理、关于）与右侧对应内容区，
  而非改动前的单栏卷动卡片流。
- [ ] **需用户手动验证**：设置页左上角存在一个「←」返回入口，点击后回到
  任务清单主视图，行为与改动前"返回清单"按钮等价或更优（不劣化）。
